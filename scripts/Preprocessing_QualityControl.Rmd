...
results_out <- 'results/Preprocessing_QualityControl/'
...
```{r barcode_rank, warning=FALSE, fig.height=11.5, fig.width=13.5, fig.caption='Barcode-rank plot for each sample. Red circles denote cell barcodes considered to be actual cells. Black circles denote empty droplets and removed from analysis.'}
...
cowplot::plot_grid(plotlist = rank_plots, ncol = 4, byrow = FALSE)
rm(gg)
```
...
```{r qc_metrics, fig.height=6, fig.width=6, fig.cap='Violin plot of various quality control metrics.'}
...
```{r doublet_results, fig.height=5, fig.width=10, fig.cap='Distribution of doublet scores. Higher scores indicate greater likelihood of doublet. Red line indicates threshold for labeling doublets.'}
...
for (id in names(doublet_results)) {
  p.doublet[[id]] <- data.frame('score'=doublet_results[[id]]$doublet_score) %>%
    ...
    geom_vline(xintercept = doublet_results[[id]]$Threshold_score,
               linetype = 'dashed', col = 'indianred', size = 1) + 
...
ggsave(filename = paste0(scrublet_out, 'score_hist.tiff'),
       plot = p.doublet, height = 5, width = 10, device = 'tiff')
p.doublet
```
